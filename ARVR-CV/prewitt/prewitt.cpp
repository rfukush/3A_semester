...
void process(IplImage *source, IplImage *destination) {
  ...
  int filter[][3] = { { -1, 0, 1 }, { -1, 0, 1 }, { -1, 0, 1 } };
  //int filter[][3] = { { -1, -1, -1 }, { 0, 0, 0 }, { 1, 1, 1 } };
  ...
  for (y = size.height / 4; y < (size.height * 3 / 4); y++) {
    ...
    for (x = size.width / 4; x < (size.width * 3 / 4); x++) {
      ...
      for (j = 0; j < 3; j++) {
        for (i = 0; i < 3; i++) {
          uchar* pN = dataS + step * (y + j - 1) + bpp * (x + i - 1) + 0;
          double intensityS_d;
          double intensityS;
          uchar blueS = *pN;
          uchar greenS = *(pN + 1);
          uchar redS = *(pN + 2);
          intensityS = (0.114 * blueS + 0.587 * greenS + 0.299 * redS) / 255.0;
          intensityS_d = (219.0 * intensityS) + 16;
          intensityD_d += intensityS_d * filter[j][i];
        }
      }
      *pD = abs(intensityD_d);
      *(pD + 1) = abs(intensityD_d);
      *(pD + 2) = abs(intensityD_d);
      pD += 3;
    }
  }
}
